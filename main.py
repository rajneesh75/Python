...
async def get_customer_info(customer_id: str) -> str:
    ...
    if not customer_info:
        return "Customer not found. I am sorry"
...
async def get_order_details(order_id: str) -> str:
    ...
    if not order:
        return f"No order found with ID {order_id}.I am sorry"
...
async def check_inventory(product_name: str) -> str:
    ...
    return "\n".join(matches) if matches else "No matching products found.I am sorry"
...
@mcp.tool()
async def get_orders_by_customer_id(customer_id: str) -> dict[str, dict[str, str]]:
