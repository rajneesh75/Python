    ...
    def tokenize(examples):
        ...
        if "question" in examples and "answer" in examples:
            ...
            for i, o in zip(examples["input"], examples["output"]):
                texts.append(f"Input: {i}\nOutput: {o}")
        elif "text" in examples:
            texts = examples["text"]
        else:
            raise KeyError("Input data must contain either ('question' and 'answer'), ('input' and 'output'),"
                           " or 'text' fields.")
...
def inference(text, model, tokenizer, max_input_tokens=1000, max_output_tokens=100):
    ...
    # Generate
    device = model.device
    generated_tokens_with_prompt = model.generate(input_ids=input_ids.to(device), max_new_tokens=max_output_tokens, )

    # Remove the prompt tokens
    generated_tokens = generated_tokens_with_prompt[0][input_ids.shape[1]:]

    # Decode only the newly generated tokens
    generated_text_answer = tokenizer.decode(generated_tokens, skip_special_tokens=True)
