...
def animate(frame):
    ...
    if iteration_count[0] < num_iterations:
        ...
        for i, solution in enumerate(solutions):
            ...
            current_height = bars2[i].get_height()
            target_height = final_count[solution]
            new_height = current_height * 0.9 + target_height * 0.1
            annotations2[i].set_text(f'{int(new_height)}')
            annotations2[i].set_position((bars2[i].get_x() + bars2[i].get_width() / 2, new_height))
...
xticks = np.arange(0, paths+1, 1)  # Adjust the range and step as needed
print(f"xticks {xticks}")
ax.set_xticks(xticks)
ax.set_xticklabels(xticks)  # Ensure all xticks are labeled
plt.ion()
bar_width = 0.35
...
yticks = np.arange(0, num_ants, 1)  # Adjust the range and step as needed
